...
func newCore(port uint16, bootStrapNode *p2p.Node, isCore bool) *Core {
	c := &Core{
		State:             STATE_INIT,
		Port:              port,
	...
	if isCore {
		log.Printf("Core address is %s\n", c.IP)
		c.cm = p2p.NewConnectionManager(c.IP, port, bootStrapNode)
	} else {
		log.Printf("Edge address is %s\n", c.IP)
		c.cm = p2p.NewConnectionManager4Edge(c.IP, port, bootStrapNode)
	}
	return c
}
