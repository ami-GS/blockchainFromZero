...
func (c *ConnectionManagerCore) handleMessage(conn net.Conn) error {
	...
	if rsp == OK_WITHOUT_PAYLOAD {
		...
		case REQUEST_CORE_LIST:
			...
			return c.SendMsgTo([]byte(nodesStr), CORE_LIST, &Node{ipv4[0], msg.MyPort, nil})
		default:
			err = c.callback(msg, &Node{ipv4[0], msg.MyPort, nil})
			if err != nil {
				return err
			}
		}
		...
		case CORE_LIST:
			if c.coreNodeSet.Len() == 1 {
				...
				if isCore := c.coreNodeSet.Has(Node{ipv4[0], msg.MyPort, nil}); !isCore {
				...
				for _, nodeStr := range nodes {
					...
					node := &Node{out[0], uint16(port), nil}
					err = c.ping(node)
					if err != nil {
...
func (c *ConnectionManagerCore) addEdge(ip string, port uint16, pubkey []byte) {
	log.Printf("Adding Edge: %s:%d", ip, port)
	c.edgeNodeSet.Add(ip, port, pubkey)
}
